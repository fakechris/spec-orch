...
| Epic | 名称 | 优先级 | 文章来源 | 说明 |
|------|------|--------|----------|------|
| A | 用户启动体验 (User Onboarding Harness) | P0 | §2 Harness + §4 ACI 工具设计 | preflight 自检、init 改造、错误消息改造、Agent 友好文档。PR #111 已修 WS 403 + websockets 依赖。 |
| B | Preflight + Selftest 验证闭环 | P1 | §2 Codex 可观测性栈 + §8 先修评测再改 Agent | selftest 冒烟测试、doctor 写结构化文件 |
| C | 评测体系成熟化 | P2 | §8 Agent 评测 | 能力评测 vs 回归测试分离、outcome-based grading、eval infra 自检 |
| D | 追踪与可观测性 | P2 | §9 追踪 Agent 执行过程 | 在线评测采样、Trace 失败模式聚类、两层评测校准 |
| E | 上下文工程深化 | P2 | §3 上下文工程 | prompt 确定性逻辑提取、compact 保留优先级、工具消息隔离 |
| F | Prompt Caching 优化 | P3 | §3 Prompt Caching | LiteLLM cache_control 审计与启用 |
| G | Skill 退化检测 | P3 | §3 Skills + 方向性张力 4 | 路由审计、模型金丝雀、效果基线 |
| H | 多 Agent 幻觉防护 | P3 | §7 多 Agent 组织 | Reviewer 独立上下文、交叉验证、outcome-based 验证 |

#### 需要研究的事项

| 编号 | 名称 | 文章来源 | 研究问题 |
|------|------|----------|----------|
| R1 | Agent Loop vs Workflow 边界 | §1 | RunController 是否需要 LLM 动态路由？Full/Standard/Hotfix 是否应从静态选择改为 LLM 推荐？ |
| R2 | 长任务跨 Session 续跑 | §6 | Daemon DLQ 重试时上下文不连续；是否需要 progress.txt 机制实现跨 run 状态传递？ |
| R3 | Prompt Injection 防护 | §10 | Linear issue 描述可包含恶意指令；是否需要 `<untrusted_content>` 标注和 builder shell 路径白名单？ |
| R4 | 五种控制模式组合 | §1 | 是否引入 Parallelization（多 builder 投票）或 Routing（按复杂度选流程）？ |
| R5 | 记忆系统完整性 | §5 | Evolvers 是否等效于 MEMORY.md 自动整合？是否缺少显式的"跨 run 学习笔记"层？ |
