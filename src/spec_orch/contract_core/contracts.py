"""Canonical contract-core model for task contracts."""
...
def assess_risk_level(
    *,
    ...
    critical_file_count = sum(1 for path in files_in_scope if _CRITICAL_FILE_PATTERNS.search(path))
...
def generate_contract_from_issue(
    ...
    *,
    ...
    if any(_CRITICAL_FILE_PATTERNS.search(path) for path in files):
