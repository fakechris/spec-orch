...
- 把 live runtime observability 补成统一的 chain substrate，并把 dashboard/runtime-chain surface 保持在可诊断状态
- 把 2026-03-31 的 runtime package 研究吸收到 `runtime_core` / `decision_core` /
  `runtime_chain` / `acceptance_runtime` 的下一阶段架构计划中

Linear 当前的 program-level 焦点已经从“补底座”切到“把真实验收跑稳”：

- `SON-331` Program Reconciliation and Stability Acceptance ✅
- `SON-339` Runtime Chain Observability and Live Traceability ✅
- 真实 full E2E reruns 与环境前置收口
...
### Milestone 1: Program Reconciliation and Stability Acceptance（SON-331） ✅ DONE
...
- `dashboard_ui` 最近一次 full run 通过
- `mission_start` 最近一次 full run 通过
- `exploratory` 最近一次 full run 通过
- `issue_start` 当前仍受环境前置阻塞：`.env` 缺失且 `SPEC_ORCH_LLM_API_KEY` 未设置

### Milestone 1.5: Runtime Chain Observability and Live Traceability ✅ DONE

**为什么要做**：真实 fresh mission / exploratory E2E 暴露了一个新的基础设施缺口。ACPX worker 本身有 telemetry，但 supervisor / acceptance / replay 等长阶段缺少统一 heartbeat 和链路级状态面，导致诊断依赖手工翻日志。这个问题已经通过 runtime-chain substrate、CLI query、acceptance reporting integration、以及 dashboard runtime-chain surface 收口。

设计文档：
- [Runtime Chain Observability Epic](2026-03-31-runtime-chain-observability-epic.md)

| 任务 | 说明 | Linear |
|------|------|--------|
| runtime chain model | 统一 `chain_id` / `span_id` / `chain_events.jsonl` / `chain_status.json` | `SON-340` |
| issue/mission lineage | 把 issue / mission / round / packet 串成同一条 root chain | `SON-341` |
| supervisor/acceptance heartbeat | 给长阶段增加 phase-level heartbeat / degraded reason | `SON-342` |
| CLI / harness query | 增加 live chain query / tail / status surfaces | `SON-343` |
| acceptance status integration | 让稳定性报告直接指向 chain 状态和 terminal reason | `SON-344` |
| dashboard surfaces | 最后补 dashboard timeline / span drilldown | `SON-345` |

**已落地结果：**

- `spec-orch chain status`
- `spec-orch chain tail`
- `spec-orch chain show <chain_id>`
- mission detail / runtime-chain API surface
- dashboard mission rail 中的 runtime-chain section

### Milestone 1.6: Runtime Package Absorption

**为什么现在做**：runtime best-practice 研究已经形成了 5 份 package 文档。
这些研究结论不要求重做 Epic 1-7，而是要求把现有 core seams 从“语义已成型”
推进到“package 更完整”。重点不是增加 acceptance judgment 范围，而是补齐
tool runtime、compaction、memory lifecycle、long-task observability 这些
基础设施 package。

设计文档：
- [Runtime Package Absorption Plan](2026-03-31-runtime-package-absorption-plan.md)
- [Agentic Runtime Best Practices](2026-03-31-agentic-runtime-best-practices.md)
- [Tool Runtime Package](2026-03-31-tool-runtime-package.md)
- [Compaction Package](2026-03-31-compaction-package.md)
- [Memory Package](2026-03-31-memory-package.md)
- [Long-Task Observability Package](2026-03-31-long-task-observability-package.md)

**状态**：runtime package absorption baseline 已完成。`runtime_core.tool_runtime`、
`runtime_core.compaction`、`runtime_core.observability`、`services.memory.lifecycle`
和 `acceptance_runtime` 的消费接缝都已进入代码并通过 package-level 验证。
...
验证：
- package matrix `106 passed`
- focused observability/runner regression matrix `10 passed`
- `ruff check`, `ruff format --check`, `mypy` 全通过
